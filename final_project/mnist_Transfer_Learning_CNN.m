...
% LOADS THE MNIST DATA AND TESTS WHATEVER MODEL IS IN THE WORKSPACE
...
New_Train = Digits_Train(ranRows(1:(split_ratio*training_size)),:);%copy the training set
New_Validation = Digits_Train(ranRows((split_ratio*training_size)+1:end),:);%copy the test set

%repeate the process above for the testing set
Digits_Data_test= Digits_Data_test';
New_Test = table(Digits_Data_test);
New_Test = [New_Test table(Digits_labels_test)];
